...
func (r *NodeRepo) Update(ctx context.Context, node *domain.Node) error {
	const q = `
		UPDATE nodes SET owner_id=$1, address=$2, token_hash=$3, api_token=$4, region=$5, status=$6,
		                 cpu_cores=$7, total_memory=$8, total_disk=$9,
		                 agent_version=$10, last_ping_at=$11, updated_at=$12
		WHERE id=$13
	`
...
func (r *NodeRepo) GetByID(ctx context.Context, id int64) (*domain.Node, error) {
	const q = `
		SELECT id, owner_id, address, token_hash, api_token, region, status,
		       cpu_cores, total_memory, total_disk, agent_version,
		       last_ping_at, created_at, updated_at
...
func scanNode(s nodeScanner) (*domain.Node, error) {
	n := &domain.Node{}
	err := s.Scan(
		&n.ID, &n.OwnerID, &n.Address, &n.TokenHash, &n.APIToken, &n.Region, &n.Status,
