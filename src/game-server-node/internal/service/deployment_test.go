...
func (s *stubRuntime) RestartContainer(ctx context.Context, containerID string, timeout time.Duration) error {
	return nil
}
...
func (s *stubRuntime) ListContainers(ctx context.Context) ([]domain.ContainerInfo, error) {
...
func (s *stubRuntime) Exec(ctx context.Context, containerID string, cmd []string, stdin io.Reader, stdout, stderr io.Writer, env []string) (int, error) {
	return 0, nil
}
...
func TestDeploymentService_StopInstance_Success(t *testing.T) {
	...
	if runtime.removed {
		t.Errorf("expected RemoveContainer NOT to be called on stop")
	}

	// Verify instance remains in storage with Stopped status
	saved, err := storage.GetInstanceByID(ctx, 1)
	if err != nil {
		t.Fatalf("expected instance to remain in storage, got err: %v", err)
	}
...
func TestDeploymentService_StopInstance_RuntimeError(t *testing.T) {
	...
	// Verify status changed to Crashed due to Docker error and instance remains in storage
	saved, err := storage.GetInstanceByID(ctx, 1)
	if err != nil {
		t.Fatalf("expected instance to remain in storage, got err: %v", err)
	}
