    ...
    <div class="devtools-content">
      ...
      <div v-else-if="activeTab === 'scenarios'" class="tab-pane scenarios-pane">
        ...
        <div class="devtools-section">
          ...
          <p class="section-desc">Доступно {{ purchaseCatalog.length }} тестовых товаров для метода purchases.getAvailableItems()</p>
          <div class="catalog-list">
            <div v-for="item in purchaseCatalog" :key="item.itemId" class="catalog-item">
              <div class="item-name">{{ item.name }}</div>
              <div class="item-price">{{ item.priceCoins }} монет (ID: {{ item.itemId }})</div>
            </div>
          </div>
        </div>
      </div>
