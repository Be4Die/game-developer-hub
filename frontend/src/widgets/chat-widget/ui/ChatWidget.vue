      ...
      <div v-if="isOpen" class="chat-panel">
        ...
        <div class="chat-content">
          ...
          >
            ...
            >
              ...
              <div class="conv-meta">
                ...
                <span
                  v-if="
                    conv.unread_count > 0 || conv.unreadCount > 0
                  "
                  class="unread-count"
                  >{{ conv.unread_count || conv.unreadCount }}</span
                >
              </div>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'unread'" class="unread-list">
            <div v-if="unreadMessages.length === 0" class="empty">
  ...
  try {
    ...
    for (const conv of convs) {
      if ((conv.unread_count || conv.unreadCount) > 0) {
        ...
        unread.push(
          ...(data.messages || []).filter((m) => !m.is_read && !m.isRead)
        );
      }
    }
