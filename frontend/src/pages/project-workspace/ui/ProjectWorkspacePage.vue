  ...
  <div class="game-workspace">
    ...
    <aside class="chat-sidebar" v-if="isDraftRoute">
      ...
      <div class="chat-messages scrollable" ref="messagesContainer">
        ...
        <div v-else-if="messages.length === 0" class="message system">
          Нет сообщений
        </div>
        <div
          v-else
...
import { getModerators, useAuth } from '@/entities/user';
...
async function initChat() {
  ...
  try {
    ...
    const conv = await chatApi.createConversation(
      moderator.id,
      moderator.name || moderator.username || 'Модератор'
    );
