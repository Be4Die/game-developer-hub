    ...
    <div class="dashboard-grid">
      <div class="dashboard-section">
        <div class="section-header">
          ...
          <span class="badge badge-warning" v-if="unreadCount > 0">{{
            unreadCount
          }}</span>
        </div>
          ...
          >
            <div class="feed-header">
              <span class="feed-author">{{
                conv.participant_name || conv.participantName
              }}</span>
              <span
                class="feed-unread"
                v-if="conv.unread_count > 0 || conv.unreadCount > 0"
              >
                {{ conv.unread_count || conv.unreadCount }} новых
              </span>
            </div>
            ...
            <div class="feed-time">
              {{
                formatTime(conv.last_message_at || conv.lastMessageAt)
              }}
            </div>
          </div>
        </div>
      ...
      <div class="dashboard-section">
        <div class="section-header">
          ...
          <span class="badge badge-info" v-if="tickets.length > 0">{{
            tickets.length
          }}</span>
        </div>
...
import { tickets, loadTickets, ticketStatusText } from '@/entities/ticket';
