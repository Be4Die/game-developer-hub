    ...
    <div class="ticket-layout">
      <div class="ticket-sidebar">
        <div class="sidebar-card">
          ...
          <div
            v-if="ticket.status === 'rejected' && ticket.rejectionReason"
        ...
        <div
          v-else-if="ticket.status === 'approved'"
          class="result-banner approved"
        >
          ✓ Игра одобрена и может быть опубликована
        </div>
        <div
          v-else-if="ticket.status === 'rejected'"
          class="result-banner rejected"
        >
          ✕ Игра отклонена
        </div>
      </div>
      ...
      <div class="chat-column">
        <div class="chat-header">
          ...
          <span class="messages-count"
            >{{ messages.length }} сообщений</span
          >
        </div>

        <div class="chat-messages" ref="chatContainer">
          <div v-if="chatLoading" class="chat-empty">Загрузка чата...</div>
          <div v-else-if="!conversationId" class="chat-empty">
            Чат с разработчиком пока недоступен
          </div>
...
import { chatApi, useConversationPolling } from '@/entities/chat';
import { useAuth } from '@/entities/user';
import { RejectTicketModal } from '@/features/review-ticket';
...
function openDevPreview() {
  const url =
    ticket.value?.devUrl || `/games/${ticket.value?.id}/dev/index.html`;
...
async function findConversation() {
  ...
  try {
    ...
    const devId = String(ticket.value.developerId);
    const conv = (data.conversations || []).find(
      (c) =>
        c.participant_id === devId ||
        c.participantId === devId ||
        String(c.participant_id) === devId
    );
    if (conv) {
...
function formatSender(msg) {
  return isOwnMessage(msg)
    ? 'Вы'
    : msg.sender_name || msg.senderName || 'Разработчик';
}
