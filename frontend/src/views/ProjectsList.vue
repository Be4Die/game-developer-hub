<template>
  <div class="page-container">
    ...
    <div class="projects-grid">
      <div class="card project-card" v-for="game in games" :key="game.id" @click="$router.push(`/projects/${game.id}`)">
        <h3>{{ game.title }}</h3>
...
.project-card { background: var(--bg-card); border-color: var(--border); cursor: pointer; }
.project-card h3 { color: var(--text-main); margin: 0 0 12px; }
.project-card:hover { transform: translateY(-4px); box-shadow: 0 10px 15px rgba(0, 0, 0, 0.1); }
.badge { background: var(--bg-app); color: var(--text-muted); padding: 4px 8px; border-radius: 12px; font-size: 0.8rem; font-weight: 600; display: inline-block; }
</style>