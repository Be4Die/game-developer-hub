<template>
  <div class="nodes-page">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="node in filteredNodes" :key="node.id"
            ...
            <td class="cell-addr"><code>{{ node.address }}</code></td>
            <td>{{ node.region || '—' }}</td>
            <td><StatusBadge :status="node.status" type="node" /></td>
            <td>{{ node.cpu_cores ? node.cpu_cores + ' ядер' : '—' }}</td>
            <td>{{ node.total_memory_bytes ? formatBytes(node.total_memory_bytes) : '—' }}</td>
            <td>{{ node.total_disk_bytes ? formatBytes(node.total_disk_bytes) : '—' }}</td>
    ...
    <div v-else class="empty-state">Нет нод{{ statusFilter !== 'all' ? ' с выбранным статусом' : '' }}</div>
...
.cell-addr code { background: var(--bg-secondary); padding: 2px 6px; border-radius: 4px; font-size: 0.82rem; }
...
.tab-btn {
  ...
  font-size: 0.88rem; font-weight: 500; color: var(--text-muted); cursor: pointer;
}
