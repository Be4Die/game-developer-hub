...
export const chatApi = {
  ...
  async getMessages(conversationId, limit = 50, offset = 0) {
    ...
    const msgs = (data.messages || [])
      ...
      // Newest first, so that callers calling .reverse() will receive chronological order
  ...
  async createConversation(participantId, participantName) {
    ...
    let conv = (data.conversations || []).find(
      (c) => c.participant_id === String(participantId) || c.participantId === String(participantId)
    )
    if (!conv) {
      const now = Math.floor(Date.now() / 1000)
      conv = {
